...
.ftr{text-align:center;padding:20px;font-size:11px;color:var(--t2);font-weight:500;border-top:1px solid var(--bd);margin-top:8px}.mtbl{width:100%;border-collapse:collapse;font-size:12px}.mtbl th{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--t2);padding:8px 12px;text-align:left;border-bottom:2px solid var(--bd);white-space:nowrap}.mtbl td{padding:9px 12px;border-bottom:1px solid var(--bd);color:var(--tx);vertical-align:middle;white-space:nowrap}.mtbl tr:last-child td{border-bottom:none}.mtbl tr:hover td{background:var(--ac)}.hi-r{display:inline-block;background:#FEE2E2;color:#B91C1C;border-radius:6px;padding:2px 7px;font-weight:700;font-size:11px}.hi-o{display:inline-block;background:#FEF3C7;color:#92400E;border-radius:6px;padding:2px 7px;font-weight:700;font-size:11px}.mes-bullet{display:inline-block;width:7px;height:7px;border-radius:50%;background:#EF4444;margin-left:5px;vertical-align:middle}.mes-tick{display:inline-block;font-size:10px;margin-left:4px;color:#10B981}@media(prefers-color-scheme:dark){.hi-r{background:#450A0A;color:#FCA5A5}.hi-o{background:#451A03;color:#FCD34D}}
...
<div class="main">
  ...
  <div class="card" id="mensualCard" style="overflow-x:auto"><h3>Resumen mensual 2026</h3><div id="mensualTable"></div></div>
  <div class="card"><h3>Evolución mensual</h3><div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start"><div><div style="font-size:11px;color:#8896A5;margin-bottom:6px;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Cantidad · objetivo 160</div><div style="position:relative;height:180px"><canvas id="cMensQty"></canvas></div><div style="display:flex;gap:14px;margin-top:8px;font-size:11px;color:#8896A5"><span style="display:flex;align-items:center;gap:4px"><span style="width:10px;height:3px;background:#D85A30;display:inline-block;border-radius:2px"></span>Objetivo 160</span><span style="display:flex;align-items:center;gap:4px"><span style="width:10px;height:10px;background:#1D9E75;display:inline-block;border-radius:2px"></span>≤160</span><span style="display:flex;align-items:center;gap:4px"><span style="width:10px;height:10px;background:#EF9F27;display:inline-block;border-radius:2px"></span>161–200</span><span style="display:flex;align-items:center;gap:4px"><span style="width:10px;height:10px;background:#E24B4A;display:inline-block;border-radius:2px"></span>&gt;200</span></div></div><div><div style="font-size:11px;color:#8896A5;margin-bottom:6px;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Monto ARS · promedio 2026</div><div style="position:relative;height:180px"><canvas id="cMensARS"></canvas></div><div style="display:flex;gap:14px;margin-top:8px;font-size:11px;color:#8896A5"><span style="display:flex;align-items:center;gap:4px"><span style="width:10px;height:3px;background:#BA7517;display:inline-block;border-radius:2px"></span>Promedio mensual</span></div></div></div></div>
...
function rMensual(){var MN=['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];var moMap={};DATA.forEach(function(r){var mo=r.f?r.f.slice(0,7):null;if(!mo||!mo.startsWith('2026'))return;if(!moMap[mo])moMap[mo]=[];moMap[mo].push(r);});var mos=Object.keys(moMap).sort();var today=tods(new Date()),todayD=new Date();var tParts=mos.map(function(m){return MN[parseInt(m.slice(5))-1];});var tRange=tParts.length>1?tParts[0]+'\u2013'+tParts[tParts.length-1]:tParts[0]||'2026';document.querySelector('#mensualCard h3').textContent='Resumen mensual \u00b7 '+tRange+' 2026 \u00b7 Fuente: Devoluciones claude';function matchT(recs,kw){return recs.filter(function(r){return(r.t||'').toLowerCase().indexOf(kw.toLowerCase())>-1;}).length;}var hdr='<table class="mtbl"><thead><tr>'+'<th>Mes</th><th>Total</th><th>ARS Reg.</th><th>Monto ARS</th><th>Prom. ARS</th>'+'<th>C. Calidad</th><th>Excepciones</th><th>Error Pago</th><th>Bot\u00f3n</th><th>Error Cobro</th>'+'<th>Top producto</th>'+'</tr></thead><tbody>';var body='';mos.forEach(function(mo){var recs=moMap[mo],tot=recs.length;var arsR=recs.filter(function(r){return r.mo==='ARS';});var monto=arsR.reduce(function(s,r){return s+pm(r.mn);},0);var promA=arsR.length>0?monto/arsR.length:0;var cal=matchT(recs,'Clausula'),exc=matchT(recs,'Excep'),epago=matchT(recs,'Error de pago'),boton=matchT(recs,'Arrepent'),ecobro=matchT(recs,'Error de cobro');var topAll=cntNorm(recs,'d'),topNm=topAll.length?topAll[0][0]:'-',topN=topAll.length?topAll[0][1]:0;if(topNm.length>26)topNm=topNm.slice(0,24)+'\u2026';var moIdx=parseInt(mo.slice(5))-1,isCur=mo===today.slice(0,7);var suffix=isCur?'<span class="mes-tick">\u2705 al '+todayD.getDate()+'/'+(todayD.getMonth()+1)+'</span>':(tot>=300?'<span class="mes-bullet"></span>':'');function p(n){return tot>0?' ('+Math.round(n/tot*100)+'%)':' (0%)';}var calPct=tot>0?Math.round(cal/tot*100):0,excPct=tot>0?Math.round(exc/tot*100):0;body+='<tr>';body+='<td><strong>'+MN[moIdx]+'</strong>'+suffix+'</td>';var totCls=tot>200?'hi-r':tot>160?'hi-o':'';body+='<td>'+(totCls?'<span class="'+totCls+'">'+tot+'</span>':'<span style="color:#10B981;font-weight:700">'+tot+'</span>')+'</td>';body+='<td>'+arsR.length+'</td>';body+='<td>'+fmt(monto)+'</td>';body+='<td>'+fmt(promA)+'</td>';body+='<td>'+(calPct>=50?'<span class="hi-r">'+cal+p(cal)+'</span>':cal+p(cal))+'</td>';body+='<td>'+(excPct>=20?'<span class="hi-o">'+exc+p(exc)+'</span>':exc+p(exc))+'</td>';body+='<td>'+epago+p(epago)+'</td>';body+='<td>'+boton+p(boton)+'</td>';body+='<td>'+ecobro+p(ecobro)+'</td>';body+='<td style="color:var(--t2);font-size:11px">'+topNm+' \u00b7 <strong>'+topN+'</strong></td>';body+='</tr>';});document.getElementById('mensualTable').innerHTML=hdr+body+'</tbody></table>';}
var cMQ=null,cMA=null;
function rMensualChart(){var MN=['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];var moMap={};DATA.forEach(function(r){var mo=r.f?r.f.slice(0,7):null;if(!mo||!mo.startsWith('2026'))return;if(!moMap[mo])moMap[mo]={count:0,ars:0};moMap[mo].count++;if(r.mo==='ARS'){var v=pm(r.mn);if(v>0)moMap[mo].ars+=v;}});var mos=Object.keys(moMap).sort();var labs=mos.map(function(m){return MN[parseInt(m.slice(5))-1];});var counts=mos.map(function(m){return moMap[m].count;});var arsVals=mos.map(function(m){return moMap[m].ars;});var arsNz=arsVals.filter(function(v){return v>0;});var arsAvg=arsNz.length?Math.round(arsNz.reduce(function(a,b){return a+b;},0)/arsNz.length):0;var topeArr=mos.map(function(){return 160;});var avgArr=mos.map(function(){return arsAvg;});var barColors=counts.map(function(v){return v>200?'#E24B4A':v>160?'#EF9F27':'#1D9E75';});if(cMQ)cMQ.destroy();cMQ=new Chart(document.getElementById('cMensQty'),{type:'bar',data:{labels:labs,datasets:[{label:'Devoluciones',data:counts,backgroundColor:barColors,borderRadius:4,borderSkipped:false,order:2},{label:'Objetivo 160',data:topeArr,type:'line',borderColor:'#D85A30',borderWidth:2,borderDash:[6,3],pointRadius:0,fill:false,order:1}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{precision:0,color:'#8896A5'},grid:{color:'rgba(0,0,0,.04)'}},x:{grid:{display:false},ticks:{color:'#8896A5'}}}}});if(cMA)cMA.destroy();cMA=new Chart(document.getElementById('cMensARS'),{type:'bar',data:{labels:labs,datasets:[{label:'Monto ARS',data:arsVals,backgroundColor:'#378ADD',borderRadius:4,borderSkipped:false,order:2},{label:'Promedio',data:avgArr,type:'line',borderColor:'#BA7517',borderWidth:2,borderDash:[6,3],pointRadius:0,fill:false,order:1}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{color:'#8896A5',callback:function(v){return '$'+(v>=1000000?(v/1000000).toFixed(1)+'M':(v/1000).toFixed(0)+'K');}},grid:{color:'rgba(0,0,0,.04)'}},x:{grid:{display:false},ticks:{color:'#8896A5'}}}}});}
