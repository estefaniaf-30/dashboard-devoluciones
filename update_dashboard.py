...
// ── INSIGHT: comisiones bajo presión (excepciones concentradas en un curso) ──
var excRows=rows.filter(function(r){return (r.t||'').toLowerCase().indexOf('excep')>-1;});
if(excRows.length>=2){
  var excByCurso=cntNorm(excRows,'d');
  if(excByCurso.length&&excByCurso[0][1]>=2){
    var excPct=tot>0?(excRows.length/tot*100).toFixed(0):0;
    var excTxt='"'+excByCurso[0][0]+'" lidera con '+excByCurso[0][1]+' excepciones';
    if(excByCurso[1]&&excByCurso[1][1]>=2)excTxt+=' · "'+excByCurso[1][0]+'" con '+excByCurso[1][1];
    excTxt+='. El '+excPct+'% del total son excepciones.';
    ins.push({icon:'🔥',cls:'r',lbl:'Comisión bajo presión',txt:excTxt});
  }
}
// ── INSIGHT: tendencias por producto vs período anterior ──
if(ptot>0&&tot>=3){
  var curMap={},prvMap={};
  cntNorm(rows,'d').forEach(function(e){curMap[e[0].toLowerCase()]={n:e[0],v:e[1]};});
  cntNorm(prows,'d').forEach(function(e){prvMap[e[0].toLowerCase()]={n:e[0],v:e[1]};});
  // Buscamos el mayor repunte (sube fuerte vs anterior)
  var maxRise=null;
  Object.keys(curMap).forEach(function(k){
    var cur=curMap[k].v,prv=(prvMap[k]||{v:0}).v;
    var diff=cur-prv;
    if(cur>=3&&diff>=2&&(!maxRise||diff>maxRise.diff))maxRise={n:curMap[k].n,cur:cur,prv:prv,diff:diff};
  });
  if(maxRise)ins.push({icon:'📈',cls:'r',lbl:'Tendencia al alza',txt:'"'+maxRise.n+'" pasó de '+maxRise.prv+' a '+maxRise.cur+' bajas vs período anterior — está escalando.'});
  // Buscamos el mayor descenso (baja fuerte vs anterior)
  var maxDrop=null;
  Object.keys(prvMap).forEach(function(k){
    var prv=prvMap[k].v,cur=(curMap[k]||{v:0}).v;
    var diff=prv-cur;
    if(prv>=3&&diff>=2&&(!maxDrop||diff>maxDrop.diff))maxDrop={n:prvMap[k].n,cur:cur,prv:prv,diff:diff};
  });
  if(maxDrop)ins.push({icon:'📉',cls:'g',lbl:'Tendencia a la baja',txt:'"'+maxDrop.n+'" bajó de '+maxDrop.prv+' a '+maxDrop.cur+' bajas vs período anterior — señal positiva.'});
}
// ── INSIGHT: tendencia interna (primera vs segunda mitad del período) ──
if(tot>=6){
  var sortedDates=Array.from(new Set(rows.map(function(r){return r.f;}))).sort();
  if(sortedDates.length>=2){
    var midDate=sortedDates[Math.floor(sortedDates.length/2)];
    var h1=rows.filter(function(r){return r.f<midDate;}),h2=rows.filter(function(r){return r.f>=midDate;});
    var h1Map={},h2Map={};
    cntNorm(h1,'d').forEach(function(e){h1Map[e[0].toLowerCase()]={n:e[0],v:e[1]};});
    cntNorm(h2,'d').forEach(function(e){h2Map[e[0].toLowerCase()]={n:e[0],v:e[1]};});
    var maxIntern=null;
    Object.keys(h2Map).forEach(function(k){
      var v2=h2Map[k].v,v1=(h1Map[k]||{v:0}).v;
      if(v2>=3&&v2>=v1*2&&(!maxIntern||v2>maxIntern.v2))maxIntern={n:h2Map[k].n,v1:v1,v2:v2};
    });
    if(maxIntern)ins.push({icon:'⚠️',cls:'r',lbl:'Acelerándose en el período',txt:'"'+maxIntern.n+'" pasó de '+maxIntern.v1+' bajas en la primera mitad a '+maxIntern.v2+' en la segunda — tendencia creciente dentro del mismo período.'});
  }
}
document.getElementById('igrd').innerHTML=ins.map(function(x){return '<div class="ic '+x.cls+'"><div class="ii">'+x.icon+'</div><div class="il">'+x.lbl+'</div><div class="it">'+x.txt+'</div></div>';}).join('');}
function getMonthWeeks(){
  var t=new Date(),yr=t.getFullYear(),mo=t.getMonth();
  var today=tods(t);
  var lastDay=new Date(yr,mo+1,0).getDate();
  var bounds=[[1,7],[8,14],[15,21],[22,28]];
  if(lastDay>28)bounds.push([29,lastDay]);
  return bounds.map(function(b,i){
    var fr=tods(new Date(yr,mo,b[0])),to=tods(new Date(yr,mo,Math.min(b[1],lastDay)));
    var started=fr<=today,done=to<today;
    return{label:'W'+(i+1),from:fr,to:to,started:started,done:done};
  });
}
function setPreset(p){var t=new Date(),from,to;window._pvFrom=null;window._pvTo=null;if(p.charAt(0)==='w'){var wi=parseInt(p.slice(1))-1,wks=getMonthWeeks(),w=wks[wi];if(!w||!w.started)return;from=w.from;to=w.to;if(wi>0){window._pvFrom=wks[wi-1].from;window._pvTo=wks[wi-1].to;}else{window._pvFrom=null;window._pvTo=null;}}else if(p==='mes'){var b=moB(t);from=b.from;to=tods(t);var prevMo=moB(new Date(t.getFullYear(),t.getMonth()-1,1));window._pvFrom=prevMo.from;window._pvTo=prevMo.to;}else if(p==='mes-ant'){var d=new Date(t.getFullYear(),t.getMonth()-1,1),b=moB(d);from=b.from;to=b.to;}else if(p==='anio'){from='2026-01-01';to=tods(t);}document.getElementById('fD').value=from;document.getElementById('fH').value=to;render();}
function init(){var tipos=Array.from(new Set(DATA.map(function(r){return r.t;}).filter(Boolean))).sort(),ts=document.getElementById('fT');tipos.forEach(function(t){var o=document.createElement('option');o.value=t;o.textContent=t;ts.appendChild(o);});var seen={},prods=[];DATA.forEach(function(r){var d=(r.d||'').trim(),k=d.toLowerCase();if(k&&!seen[k]){seen[k]=true;prods.push(d);}});prods.sort();var ps=document.getElementById('fP');prods.forEach(function(d){var o=document.createElement('option');o.value=d;o.textContent=d;ps.appendChild(o);});document.querySelectorAll('.pbt').forEach(function(b){b.addEventListener('click',function(){document.querySelectorAll('.pbt').forEach(function(x){x.classList.remove('active');});b.classList.add('active');setPreset(b.dataset.p);});});['fD','fH'].forEach(function(id){document.getElementById(id).addEventListener('change',function(){document.querySelectorAll('.pbt').forEach(function(x){x.classList.remove('active');});window._pvFrom=null;window._pvTo=null;render();});});document.getElementById('fT').addEventListener('change',render);document.getElementById('fP').addEventListener('change',render);var wkBtns=document.getElementById('wkBtns');var wks=getMonthWeeks();var lastCompletedW=-1;wks.forEach(function(w,i){var btn=document.createElement('button');btn.className='pbt';btn.dataset.p='w'+(i+1);var mo=['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];var fd=new Date(w.from+'T12:00:00'),td=new Date(w.to+'T12:00:00');var rangeLabel=fd.getDate()+'/'+('0'+(fd.getMonth()+1)).slice(-2)+'-'+td.getDate()+'/'+('0'+(td.getMonth()+1)).slice(-2);btn.title=rangeLabel;if(!w.started){btn.textContent=w.label;btn.disabled=true;btn.style.opacity='0.35';btn.style.cursor='not-allowed';}else if(!w.done){btn.textContent=w.label+' ∙';btn.title=rangeLabel+' (en curso)';}else{btn.textContent=w.label;lastCompletedW=i;}btn.addEventListener('click',function(){document.querySelectorAll('.pbt').forEach(function(x){x.classList.remove('active');});btn.classList.add('active');setPreset(btn.dataset.p);});wkBtns.appendChild(btn);if(i<wks.length-1){var sp=document.createElement('span');sp.style.fontSize='10px';sp.style.color='var(--bd)';sp.style.margin='0 1px';sp.textContent='';wkBtns.appendChild(sp);}});var defaultPreset=lastCompletedW>=0?'w'+(lastCompletedW+1):'mes';var allBtns=document.querySelectorAll('.pbt');allBtns.forEach(function(b){if(b.dataset.p===defaultPreset)b.classList.add('active');});setPreset(defaultPreset);}
init();
</script>
</body>
</html>""")
